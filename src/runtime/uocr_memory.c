...
static int estimate_vision_memory_for_shape_internal(uint32_t max_view_size,
     ...
     * chunk-batched CLIP transformer/final tokens, chunk concat, projected
     * rows, and final formatted rows.
    ...
    uint64_t chunk_view_capacity = ((uint64_t)max_chunk_projected_rows + projected_tokens_per_view - 1u) /
                                   projected_tokens_per_view;
    if (chunk_view_capacity == 0u) {
        chunk_view_capacity = 1u;
    }
