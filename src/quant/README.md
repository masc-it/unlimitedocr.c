...
`Q2_K` and `IQ2_XXS` are kept disabled until their converter policy and Metal
kernels are implemented and validated. `PADDED_Q4_K` is implemented as an
opt-in Metal kernel contract, but the default converter policy still leaves it
disabled until calibration proves the tradeoff is worthwhile.
...
`Q8_0` until padded q4 is enabled explicitly by a later calibrated policy.
