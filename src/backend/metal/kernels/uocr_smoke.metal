...
// Generated from: common.metal, dense.metal, dense_q8.metal, norm.metal, sam.metal, sam_q8.metal, sam_attention.metal, sam_position.metal, attention_decode.metal, attention_prefill.metal, attention.metal, attention_q8.metal, clip.metal, clip_q8.metal, clip_sam.metal, projector_q8.metal, embedding.metal, embedding_q8.metal, kv_cache.metal, layout.metal, moe.metal, moe_q8.metal, prompt_assembly.metal, rope.metal, sampling.metal, sam_conv.metal, sam_window.metal, smoke.metal, lm_head_q8.metal, mpp_prototypes.metal
...
// clip_q8.metal - Fused Q8_0 CLIP transformer linear kernels

#define UOCR_CLIP_Q8_ACT_NONE 0u
#define UOCR_CLIP_Q8_ACT_QUICKGELU 1u

struct UocrClipLinearQ8Params {
    uint n_rows;
    uint input_size;
    uint output_size;
    uint group_size;
...
// Fused Q8_0 QKV projection: one dot product per output column of the packed
// [3*hidden, hidden] weight, written directly into split Q/K/V buffers with
// bias applied.  Replaces MPS matmul + bias add + split for Q8 blocks.
[[max_total_threads_per_threadgroup(1024)]] kernel void uocr_clip_qkv_tile4_q8_0_to_f16(
    device const half *src [[buffer(0)]],
    device const char *qweight [[buffer(1)]],
    device const half *qscale [[buffer(2)]],
    device const half *bias [[buffer(3)]],
    ...
    constant UocrClipLinearQ8Params &params [[buffer(7)]],
    threadgroup float *partials [[threadgroup(0)]],
    uint output_tile [[threadgroup_position_in_grid]],
    uint tid [[thread_index_in_threadgroup]],
    uint simd_width [[threads_per_simdgroup]]) {
    const uint tile_columns = 4u;
    const uint threads_per_column = 256u;
    const uint hidden_size = params.input_size;
    const uint qkv_size = params.output_size;
    const uint tiles_per_row = uocr_div_up_u32(qkv_size, tile_columns);
    const uint row = output_tile / tiles_per_row;
    const uint tile = output_tile - row * tiles_per_row;
    const uint col_group = tid / threads_per_column;
    const uint local_tid = tid - col_group * threads_per_column;
    ...
    const uint group_size = params.group_size;
    const uint groups_per_row = params.groups_per_row;
    const uint src_base = row * hidden_size;
    ...
    const float value = uocr_threadgroup_sum(sum,
                                             ...
                                             threads_per_column,
                                             simd_width);
    if (local_tid == 0u && out_col < qkv_size) {
        const uint projection = out_col / hidden_size;
        const uint col = out_col - projection * hidden_size;
        device half *dst = projection == 0u ? q_dst : (projection == 1u ? k_dst : v_dst);
        dst[row * hidden_size + col] = half(value + float(bias[out_col]));
    }
}

// Fused Q8_0 linear: dst[row, out_col] = act(dot(src[row, :], w[out_col, :]) + bias[out_col]).
// Four output columns per 1024-thread threadgroup; fp32 accumulation.
[[max_total_threads_per_threadgroup(1024)]] kernel void uocr_clip_linear_tile4_q8_0_to_f16(
    device const half *src [[buffer(0)]],
    device const char *qweight [[buffer(1)]],
    device const half *qscale [[buffer(2)]],
    device const half *bias [[buffer(3)]],
    device half *dst [[buffer(4)]],
    constant UocrClipLinearQ8Params &params [[buffer(5)]],
    threadgroup float *partials [[threadgroup(0)]],
    uint output_tile [[threadgroup_position_in_grid]],
    uint tid [[thread_index_in_threadgroup]],
    uint simd_width [[threads_per_simdgroup]]) {
    ...
    if (local_tid == 0u && out_col < params.output_size) {
        value += float(bias[out_col]);
        if (params.activation == UOCR_CLIP_Q8_ACT_QUICKGELU) {
