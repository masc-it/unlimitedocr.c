...
> methodology that shipped Q8 and Q4: **probe-first kernels → cfg flip →
> manual notebook QA → commit → tick the checkbox here**.  Every step below
> ends with a config flip and a user QA gate; no step's checkbox is ticked
> until QA passes on the real path.  The quality gate is the manual QA on the
> real converted model: the format is chosen up front from established
> practice and the fallback ladder (§7) absorbs a QA regression.
...
QA essentially for free (Q4_0 dequant rmse ~0.0022 on the expert weights).
The literature and
...
## 1. Q2_K format and `.uocr` metadata

The packing details are frozen only after the §4 probe confirms the layout.
...
* [ ] `_quantize_q2_k_rows()` + `_stream_bf16_to_q2_k()` writer: per
      ...
      sub-scales/mins, crumb packing (exact packing frozen by the §4 probe).
* [ ] Round-trip unit tests: pack/unpack identity, all-zero and all-equal
      ...
      written byte counts; record the dequant rmse per expert projection
      (reference point: Q4_0 measured ~0.0022).
...
* [ ] Engine open accepts `UOCR_QPROFILE_MIXED_Q2` **only in the §6
      commit** (fail loudly until kernels exist — same deferral Q4 used).
* [ ] Commit; tick.

---

## 4. Metal kernels (probe-first, per `docs/howto_quant.md` §6)

### 4.1 Packing probe — before freezing §1
...
### 4.2 Decode GEMV (`decode_gemv_q2.metal`)
...
### 4.3 Prefill bucketed GEMM (`gemm_q2.metal`)
...
## 6. Rollout: cfg flip + manual QA (the gate)
...
## 7. Fallback ladder (if §6 QA regresses)
...
* **Embeddings, shared experts, dense MLP**: run for every token with no
  ...
  margin, and then as their own gated module steps.
...
Reporting:
  converter dequant-rmse table, packing-probe table, per-qtype byte
  totals, decode
