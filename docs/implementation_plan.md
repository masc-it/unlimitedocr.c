...
- [x] Keep only one production vision path after timing confirms the GPU-resident implementation; delete the host-scratch production path.
- [x] Preserve diagnostic parity entrypoints only if they are clearly out of the production path and do not keep slow fallback code alive.
...
- [x] Reuse input/output NDArray wrappers where buffer lifetimes allow.
- [x] Measure whether destination zero-fill blits are required; remove them if confirmed unnecessary.
